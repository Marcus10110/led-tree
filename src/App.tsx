...
const computeLocations = (data: Pick<TreeData, 'ledCount' | 'ledLengthPx' | 'treeBasePx' | 'treeHeightPx'>, size: { width: number; height: number} = { width: 500, height: 500}): TreeData =>
{
  ...
  for(let i = 0; i < data.ledCount; ++i)
  {
    ...
    const verticalShift = (((currentLength - rowStart) / rowLength)-.5) * stripePitch / 1.5 * (rowLength/squareHeight);
    
    const horizontalShift = (squareHeight - rowLength) / 2;
    ledLocations.push({ x: (currentLength - rowStart + horizontalShift) / Math.PI + 10, y: (verticalShift * (i % 2 === 0 ? 1 : -1)) + size.height - (stripePitch * rowIndex) - 10 - stripePitch });
  }
...
const drawTree = (data: TreeData, drawFn: LedFn, context: CanvasRenderingContext2D) => {
  ...
  if( data.ledCount !== data.ledPositions.length || data.ledCount !== colorData.length) throw Error('LED count mismatch');
  for(let i = 0; i < data.ledCount; ++i)
  {
    const { x, y} = data.ledPositions[i];
